...
const AUTH_CONTEXT_MAX_ATTEMPTS = 4;
const AUTH_CONTEXT_RETRY_DELAY_MS = 250;
...
function isTransientAuthContextDbError(error: unknown): boolean {
...
export async function loadAuthContext(userId: string): Promise<AuthContext | null> {
  for (let attempt = 1; attempt <= AUTH_CONTEXT_MAX_ATTEMPTS; attempt += 1) {
    try {
      return await loadAuthContextOnce(userId);
    } catch (error) {
      if (attempt < AUTH_CONTEXT_MAX_ATTEMPTS && isTransientAuthContextDbError(error)) {
        await wait(AUTH_CONTEXT_RETRY_DELAY_MS);
        continue;
      }
      throw error;
    }
...
async function loadAuthContextOnce(userId: string): Promise<AuthContext | null> {
